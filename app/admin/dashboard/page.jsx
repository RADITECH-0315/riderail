...
export default function AdminDashboardPage() {
  ...
  return (
    <div className="max-w-6xl mx-auto p-6 mt-6">
      <h1 className="text-3xl font-bold text-slate-900 mb-6 text-center">📊 Admin Dashboard</h1>

      {/* KPIs */}
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
        <div className="bg-white rounded-2xl shadow-md p-6 text-center">
          <h2 className="text-lg font-semibold text-gray-600">Total Revenue (This Month)</h2>
          <p className="text-2xl font-bold text-green-600 mt-2">₹{data.totalRevenue || 0}</p>
        </div>
        <div className="bg-white rounded-2xl shadow-md p-6 text-center">
          <h2 className="text-lg font-semibold text-gray-600">Completed Rides (This Month)</h2>
          <p className="text-2xl font-bold text-blue-600 mt-2">{data.completedRides || 0}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-md p-6 mb-8">
        <div className="flex items-center justify-between">
          <h2 className="text-lg font-semibold text-gray-700">Rides per Day</h2>
          <Link href="/admin/bookings" className="text-sm text-[var(--brand)] underline">
            View bookings →
          </Link>
        </div>
        ...
        {data.ridesPerDay?.length ? (
          ...
          <p className="text-center text-gray-500 mt-6">No rides this month yet.</p>
        )}
      </div>
                ...
                {latest.map((b) => {
                  ...
                  return (
                    <tr key={b._id} className="border-b last:border-0">
                      ...
                      <td className="py-2 pr-4">{b.customer?.phone || "—"}</td>
                      <td className="py-2 pr-4">{shortPickup} → {shortDrop}</td>
